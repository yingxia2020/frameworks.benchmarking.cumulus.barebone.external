...
class KubernetesVirtualMachine(virtual_machine.BaseVirtualMachine):
  ...
  def _Create(self):
    self._CreatePod()
    if(self.image.find('kafkabroker') != -1):
       self._CreateService()
  ...
  def _CreateService(self):
    ...
    Creates a service .
    """
    create_ser_body = self._BuildServiceBody()
    logging.info('About to create a service with the following configuration:')
    logging.info(create_ser_body)
    kubernetes_helper.CreateResource(create_ser_body)
  ...
  def _DeleteService(self):
    ...
    delete_service = [FLAGS.kubectl, '--kubeconfig=%s' % FLAGS.kubeconfig,
                      'delete', 'svc', 'e2e-kafkabroker-0']
    stdout, _, _ = vm_util.IssueCommand(delete_service)
    logging.info(stdout.rstrip())
  ...
  def _ConfigureProxy(self):
    ...
    self.image = self.image or self.DEFAULT_IMAGE
    if (self.image.startswith("centos")) | (self.os_type == "rhel"):
      bash_var = "~/.bashrc"
    elif (self.image.startswith("ubuntu")) | (self.os_type.startswith("ubuntu")):
      bash_var = "/etc/bash.bashrc"
    if FLAGS.http_proxy:
      http_proxy = 'sed -i \'1i export http_proxy=%s\' %s'
      self.RemoteCommand(http_proxy % (FLAGS.http_proxy, bash_var))
    if FLAGS.https_proxy:
      https_proxy = 'sed -i \'1i export https_proxy=%s\' %s'
      self.RemoteCommand(https_proxy % (FLAGS.http_proxy, bash_var))
    if FLAGS.ftp_proxy:
      ftp_proxy = 'sed -i \'1i export ftp_proxy=%s\' %s'
      self.RemoteCommand(ftp_proxy % (FLAGS.ftp_proxy, bash_var))
  ...
  def _BuildPodBody(self):
    ...
    if FLAGS.k8s_node_affinity_label:
      for node_affinity_label in FLAGS.k8s_node_affinity_label:
        group, key_and_value = node_affinity_label.split(":")
        ...
        if group == self.group_name:
          ...
          template['spec']['affinity'].update(node_affinity)
  ...
  def _BuildServiceBody(self):
    ...
    to Kuberneres API in order to create a Service.
    All ports are defined and opened for kafka & zookeper communication
    """
  ...
  def _BuildContainerBody(self):
    ...
    container = {
        ...
        'env': [
            {
                'name': 'http_proxy',
                'value': FLAGS.http_proxy
            },
            {
                'name': 'https_proxy',
                'value': FLAGS.https_proxy
            },
            {
                'name': 'ftp_proxy',
                'value': FLAGS.ftp_proxy
            },
        ],
    }
...
def _InstallSudoCommand() -> str:
  ...
  return ' && '.join([
      ...
      'sed -i \'1i Defaults !env_reset\' /etc/sudoers',
