...
flags.DEFINE_enum('edp_script_type', 'ruby',
                  ['ruby', 'python3'], 'Optional, default is ruby. eg --edp_script_type=python3 if need to use python script')
...
def _Install(vm):
  ...
  # install emon
  vm.RemoteCommand(f'sudo rm -rf {0} && sudo mkdir -p {0}'.format(EMON_MAIN_DIR))
...
def _PostProcess(vm):
  ...
  if FLAGS.trace_skip_install and _PostProcessingPackagesExist(vm):
    ...
    vm.RemoteCommand('sudo pip3 install xlsxwriter')
    if FLAGS.edp_script_type == 'ruby':
      logging.info("Installing ruby ...")
      vm.Install('ruby')
    elif FLAGS.edp_script_type == 'python3':
      vm.RemoteCommand('sudo pip3 install pandas numpy pyyaml xmltodict')
...
def FetchResults(vm):
  ...
  if not FLAGS.emon_post_process_skip:
    tar_pkgs.append(('*edp*.csv', EMON_EDP_TARBALL))
