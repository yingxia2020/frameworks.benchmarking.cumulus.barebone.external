...
def YumInstall(vm):
  ...
  vm.RemoteCommand("sudo yum install -y --enablerepo=extras epel-release")
  vm.RemoteCommand("sudo yum install -y dkms")
  vm.RemoteCommand("sudo yum install -y habanalabs-firmware-{0} habanalabs-firmware-tools-{0}".
...
def AptInstall(vm):
  vm.RemoteCommand('curl -X GET https://vault.habana.ai/artifactory/api/gpg/key/public | sudo apt-key add - && echo "deb https://vault.habana.ai/artifactory/debian $(grep VERSION_CODENAME= /etc/os-release | cut -f2 -d=) main" | sudo tee -a /etc/apt/sources.list.d/artifactory.list > /dev/null && sudo dpkg --configure -a && sudo apt-get update -y')
  vm.RemoteCommand("sudo apt-get install -y dkms libelf-dev")
  vm.RemoteCommand("sudo apt-get install -y habanalabs-firmware={0} habanalabs-firmware-tools={0}".format(FLAGS.habana_version))
  vm.RemoteCommand("sudo apt-get install -y --allow-downgrades habanalabs-thunk={0} habanalabs-dkms={0} habanalabs-graph={0} habanalabs-container-runtime={0}".format(FLAGS.habana_version))
  vm.RemoteCommand("sudo modprobe habanalabs_en")
  vm.RemoteCommand("sudo modprobe habanalabs")
